...
const columns: DataTableColumns<Api.Central.EventSummary> = [
  {
    ...
    width: 80,
    render: row =>
      h(NTag, { size: 'small', type: severityType(row.severity) }, { default: () => severityLabel(row.severity) })
  },
    ...
    render: row => {
      ...
      if (isRenewal(row)) {
        ...
        buttons.push(
          h(
            NButton,
            { size: 'small', type: 'primary', loading: actionLoading.value === row.id, onClick: () => retrySync(row) },
            { default: () => '重新同步' }
          ),
          h(
            NButton,
        ...
        buttons.push(
          h(
            NButton,
            { size: 'small', loading: actionLoading.value === row.id, onClick: () => resolve(row) },
            { default: () => '标记已处理' }
          )
        );
      }
