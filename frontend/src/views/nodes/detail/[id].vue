...
function openNameEditor() {
  if (!detail.value) return;
  nameError.value = '';
  nodeName.value = detail.value.name;
  nameModalVisible.value = true;
}

function closeNameEditor() {
  if (!nameSaving.value) nameModalVisible.value = false;
}

async function saveNameEditor() {
  const value = nodeName.value.trim();
  if (!value) {
    nameError.value = '请填写节点名称';
    return;
  }
  if (value.length > 120) {
    nameError.value = '节点名称不能超过 120 个字符';
    return;
  }
  if (!detail.value || nameSaving.value) return;
  nameError.value = '';
  nameSaving.value = true;
  const { error } = await updateNode(detail.value.id, { name: value });
  nameSaving.value = false;
  if (error) {
    nameError.value = '节点名称保存失败，请稍后重试。';
    return;
  }
  nameModalVisible.value = false;
  window.$message?.success('节点名称已更新');
  await loadNode();
}
  ...
  <div class="node-detail-page">
    ...
    >
      ...
      <template v-if="detail">
        <div class="grid gap-16px lg:grid-cols-4">
          <NCard size="small" class="lg:col-span-3">
            <div class="flex flex-wrap items-start justify-between gap-12px">
              ...
              <NSpace>
                ...
                <NButton size="small" secondary @click="openNameEditor">编辑节点名称</NButton>
            ...
            <NDescriptions class="mt-16px" label-placement="left" :column="2" bordered size="small">
              ...
              <NDescriptionsItem label="X-Panel 同步">
                <NSpace align="center" :size="8">
                  <SyncStatusTag :status="detail.syncStatus" />
                  <span v-if="detail.lastSyncError" class="text-12px text-red-500">{{ detail.lastSyncError }}</span>
                </NSpace>
              </NDescriptionsItem>
              <NDescriptionsItem label="Xray 版本">
                {{ detail.xrayVersion || '未检测到' }}
              </NDescriptionsItem>
          ...
          <NCard size="small" title="当前汇总">
            <div class="space-y-14px">
              ...
              <div class="flex items-center justify-between">
                <span class="text-gray-500">收费 / 免费</span>
                <span class="font-600">{{ detail.paidUserCount }} / {{ detail.freeUserCount }}</span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-gray-500">启用 Client</span>
                <span class="font-600">{{ detail.clientCount }}</span>
              </div>
              <div class="flex items-center justify-between">
        ...
        <NAlert v-else-if="detail.syncStatus === 'failed'" type="warning" :show-icon="true" class="mt-16px">
          Agent 仍可连接中央服务，但 X-Panel 数据采集失败。{{
            detail.lastSyncError || '请检查节点上的 X-Panel 地址、API 路径和凭据。'
          }}
        </NAlert>
    ...
    <NModal
      ...
      :mask-closable="false"
    >
      <NAlert v-if="nameError" type="error" :show-icon="true" class="mb-16px">{{ nameError }}</NAlert>
      <NForm label-placement="top">
        <NFormItem label="节点名称" required>
          <NInput
            ...
            maxlength="120"
            show-count
            placeholder="例如：东京线路机"
            @keyup.enter="saveNameEditor"
          />
        </NFormItem>
      </NForm>
      <template #footer>
        <NSpace justify="end">
          <NButton :disabled="nameSaving" @click="closeNameEditor">取消</NButton>
          <NButton type="primary" :loading="nameSaving" @click="saveNameEditor">保存</NButton>
        </NSpace>
      </template>
    </NModal>
