...
const columns: DataTableColumns<Api.Central.NodeSummary> = [
  ...
  },
  {
    ...
    key: 'syncStatus',
    width: 120,
    render: row => h(SyncStatusTag, { status: row.syncStatus })
  },
  { title: 'Xray', key: 'xrayVersion', minWidth: 110, render: row => row.xrayVersion || '--' },
  {
    ...
    key: 'userCount',
    width: 110,
    render: row => (row.type === 'relay' ? `${row.userCount} 人` : '--')
  },
  {
    title: '收费 / 免费',
    key: 'paidUserCount',
    width: 130,
    render: row => (row.type === 'relay' ? `${row.paidUserCount} / ${row.freeUserCount}` : '--')
  },
  {
    title: '启用 Client',
    key: 'clientCount',
    width: 120,
    render: row => (row.type === 'relay' ? `${row.clientCount} 台` : '--')
  },
...
<template>
  <div class="nodes-page">
    <ModulePage
      title="节点管理"
      description="查看线路机和落地机的健康状态、Xray 版本、同步时间、承载用户与节点成本。"
      :loading="loading"
      :error="errorMessage"
      :empty="rows.length === 0"
