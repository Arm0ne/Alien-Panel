...
const bindingForm = reactive({
  exitIpId: '',
  scope: 'landing' as Api.Central.RouteExitIpScope,
  allocationWeight: 1 as number | null
});
const bindingDrafts = reactive<
  Record<string, { scope: Api.Central.RouteExitIpScope; allocationWeight: number | null; enabled: boolean }>
>({});
const statusOptions = [
...
const bindingExitIpOptions = computed(() => {
  ...
  return bindingExitIps.value
    ...
    .map(item => ({
      label: `${item.address} · ${item.ownerNodeName || '独立 S5'} · ${item.provider || '未填写服务商'}`,
      value: item.id
    }));
});
const bindingModalTitle = computed(() =>
  bindingRoute.value ? `出口 IP 配置 · ${bindingRoute.value.name}` : '出口 IP 配置'
);
...
async function saveBinding() {
  ...
  if (
    bindingForm.allocationWeight === null ||
    !Number.isFinite(bindingForm.allocationWeight) ||
    bindingForm.allocationWeight <= 0
  ) {
      ...
      h(
        ...
        { size: 4 },
        {
          default: () => [
            h(
              NButton,
              { size: 'small', type: 'primary', text: true, onClick: () => void openEdit(row.id) },
              { default: () => '编辑' }
            ),
            h(
              NButton,
              { size: 'small', type: 'info', text: true, onClick: () => void openBindings(row) },
              { default: () => '出口 IP' }
            ),
            h(
              NButton,
              {
                ...
                type: 'error',
                text: true,
                loading: deletingRouteId.value === row.id,
                disabled: Boolean(deletingRouteId.value) && deletingRouteId.value !== row.id,
...
const bindingColumns: DataTableColumns<Api.Central.RouteExitIpBinding> = [
  ...
  },
  {
    ...
    key: 'ownerNodeName',
    minWidth: 150,
    render: row => (row.sourceType === 's5' ? '独立 S5' : row.ownerNodeName || '--')
  },
  { title: '区域', key: 'region', minWidth: 110, render: row => row.region || '--' },
  {
    title: '分配权重',
          ...
          default: () => [
            h(
              NButton,
              {
                ...
                type: 'primary',
                text: true,
                loading: bindingUpdatingId.value === row.id,
                onClick: () => void saveBindingRow(row)
              },
              { default: () => '保存' }
            ),
            h(
              NButton,
              {
                ...
                type: 'error',
                text: true,
                loading: bindingDeletingId.value === row.id,
                disabled: Boolean(bindingDeletingId.value) && bindingDeletingId.value !== row.id,
                onClick: () => confirmRemoveBinding(row)
              },
              { default: () => '解绑' }
            )
          ]
      ...
      <NSpin :show="modalLoading">
        ...
        <NForm label-placement="top">
          <div class="grid gap-x-12px md:grid-cols-2">
            ...
            <NFormItem label="状态">
              <NSwitch v-model:value="routeForm.enabled">
            ...
            <NFormItem label="线路机" required>
              <NSelect
                v-model:value="routeForm.relayNodeId"
            ...
            <NFormItem label="落地机" required>
              <NSelect
                v-model:value="routeForm.landingNodeId"
                :options="landingNodeOptions"
                :loading="nodesLoading"
                filterable
                clearable
                placeholder="选择落地机"
            ...
            <NFormItem label="生效日期">
              <NDatePicker
                v-model:formatted-value="routeForm.validFrom"
                type="date"
            ...
            <NFormItem label="失效日期">
              <NDatePicker
                v-model:formatted-value="routeForm.validTo"
                type="date"
                value-format="yyyy-MM-dd"
                clearable
                class="w-full"
      ...
      <NSpin :show="bindingLoading">
        ...
        <NCard title="新增出口 IP 绑定" size="small" :segmented="{ content: true }">
          <NSpace align="center" wrap>
            <NSelect
              v-model:value="bindingForm.scope"
              :options="bindingScopeOptions"
              :disabled="bindingSaving"
              class="w-150px"
            />
            <NSelect
              v-model:value="bindingForm.exitIpId"
              :options="bindingExitIpOptions"
              :disabled="bindingSaving"
              :loading="bindingLoading"
              filterable
              clearable
              class="w-360px"
              placeholder="选择符合出口位置的资产"
            />
            <NInputNumber
              v-model:value="bindingForm.allocationWeight"
              :min="0.01"
              :max="1000000"
              :precision="2"
              :disabled="bindingSaving"
              ...
              placeholder="分配权重"
            />
            <NButton type="primary" :loading="bindingSaving" :disabled="bindingLoading" @click="saveBinding">
              绑定
            </NButton>
          </NSpace>
        </NCard>
