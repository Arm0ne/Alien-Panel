...
const state = computed(() => {
  ...
  if (age > expiredAfter) {
    return {
      label: '数据过期',
      type: 'error' as const,
      detail: `已超过 ${Math.round(expiredAfter / 60000)} 分钟未同步`
    };
  }
  if (age > delayedAfter) {
    return {
      ...
      detail: `已超过 ${Math.round(delayedAfter / 60000)} 分钟未同步`
    };
  }
  ...
  <NTooltip>
    <template #trigger>
      <NTag size="small" :type="state.type">
        {{ state.label }}
        <span v-if="!compact && formattedAt" class="ml-4px">· {{ formattedAt }}</span>
      </NTag>
    </template>
