...
func TestCompatibilityFixtures(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !status.XrayRunning || status.XrayVersion != test.statusXray || status.CPUUsage != test.statusCPU || status.MemoryUsed != test.statusMemoryUsed || status.DiskUsed != test.statusDiskUsed {
				t.Fatalf("status = %+v", status)
			}
			if encoded, err := json.Marshal(status); err != nil {
				t.Fatalf("marshal status: %v", err)
			} else if strings.Contains(string(encoded), "xpanel_version") {
				t.Fatalf("status still exposes xpanel_version: %s", encoded)
			}
		})
	}
}
