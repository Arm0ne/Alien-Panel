...
func (snapshot Snapshot) Heartbeat() Heartbeat {
	return Heartbeat{NodeKey: snapshot.NodeKey, ObservedAt: snapshot.ObservedAt, Status: snapshot.Status, StatusAvailable: true}
}
...
// AgentHeartbeat reports that the Agent itself can reach the central service.
// It intentionally does not read X-Panel, so a local panel failure cannot
// make an otherwise connected Agent appear offline.
func (collector *Collector) AgentHeartbeat(syncState, syncError string) Heartbeat {
	errorText := strings.TrimSpace(syncError)
	...
	return Heartbeat{
		...
		Status:     Status{AgentVersion: collector.agentVersion},
