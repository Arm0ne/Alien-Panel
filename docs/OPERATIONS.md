...
- 已归档 Inbound 重新出现在成功同步中时，如果它原来关联的用户已经被软删除，中央会自动结束旧关联、清理旧运营状态并创建新用户；旧用户的账务记录和审计历史保留，不会再次复用已删除用户。
- 线路机同步到新用户 Inbound 后，事件铃铛会弹出“发现新增用户，待完善资料”。进入用户管理保存业务名称、收费类型、周期和金额后，该事件自动完成；重复同步不重复创建提醒，落地机 Inbound 不触发此流程。
...
事件中心记录新增用户待完善、同步异常、节点离线/恢复、Inbound 缺失/归档、流量回退和待确认续费。查看中央日志：
