...
2. 需要多个公网出口时，在向导一次填写出口 IP 列表；管理地址 `managementUrl` 只用于打开 X-Panel/运维入口，不替代出口资产。旧数据中的 `publicIp` 仅作兼容展示。
...
Agent 会将 X-Panel 的 Inbound/Client 到期字段统一解析为中央时间格式，兼容 Unix 秒、毫秒和日期字符串。业务用户仍按线路机 Inbound 统计，但用户到期和状态只使用该 Inbound 的已启用 Client：多个设备时间一致时直接使用，不一致时按最早有限时间计算并在事件中心提示；Inbound 停用或没有启用 Client 时用户显示为停用。升级解析逻辑后，需在节点重新执行前台生成的一键安装命令，或替换 `/usr/local/bin/xpanel-agent` 后执行 `systemctl restart xpanel-agent`。

## 3. 日常管理

- “节点管理”中的停用会立即拒绝该节点的 heartbeat、sync 和立即同步请求；重新启用后恢复认证。
- 删除节点会永久清理节点及其 Inbound、Client、同步记录、流量快照、成本、出口 IP、用户路径和旧线路兼容记录；业务用户本身不会删除。删除后 Agent Token 失效，原 Node Key 可以重新使用。
- 用户详情中的“用户路径”只更新中央配置：线路机从用户主 Inbound 自动确定，管理员可选落地机/落地 Inbound，并指定一个固定出口 IP；不再要求先创建线路模板或出口池。
- 出口位置分为线路机直出、落地机出口和独立 S5。出口 IP 的归属节点由资产自动显示，独立 S5 不属于任何 VPS。
- 旧线路关系页面和 `/api/users/{id}/route` 接口保留用于历史数据/兼容客户端，但不再是日常用户分配入口；新配置写入 `user_paths`。
- 页面中的“配置归属用户数”是当前有效配置统计，不等同于实际产生流量的用户数。
- 用户详情可维护月付/年付及周期价格。Agent 发现线路机 Inbound 下的 Client 派生到期时间延长时只生成“待确认续费”，不会自动算作收款；管理员确认后才写入收费历史并进入财务统计，选择“非收费变更”则只关闭候选。
- 年费按收费记录的实际服务区间分摊到各统计月份；未产生收费记录的旧用户继续使用 `monthly_fee` 兼容口径。删除或停用节点不会删除用户收费历史。
- “事件中心”只展示业务事件和有效告警。立即同步请求保留在内部 `sync_runs`/内部事件中，不作为管理员事件列表；Agent 在线但 X-Panel 采集失败会显示为同步异常，完整同步恢复后自动清除；节点离线/恢复、流量回退和 Inbound 异常也会形成可检索事件。
- 待确认续费会在事件中心进入“待处理”，并在右上角消息按钮显示红点；登录后或已登录期间发现新待办会强提醒。标记已读不会关闭待办，必须确认续费或标记为非收费变更后才会清除待处理状态。
...
恢复命令会先验证源文件、创建恢复前安全快照，再原子替换数据库并复核完整性。恢复后应检查 Dashboard 数据时间、节点数量、用户路径/历史和最近同步记录。
