...
- panel_base_path
- agent_version
- xpanel_version       -- legacy nullable column; no longer collected or exposed
- xray_version
- enabled
...
当前实现补充：用户详情接口继续保留最近 30 条累计快照字段以兼容旧客户端，并新增
`GET /api/users/:id/traffic?range=1h|6h|1d|7d`。趋势接口只读取线路机业务 Inbound，
按相邻累计快照计算上传/下载增量和速率，再按 1 分钟、5 分钟、15 分钟、1 小时聚合；
页面主图展示从所选时间范围基线开始累加的总流量增长曲线，并辅以上传/下载累计曲线；
速率只在悬浮提示和摘要中展示，重置/采样缺口会明确提示，落地机基础设施 Inbound 不会计入业务用户流量。
业务字段编辑仅写中央数据库并记录审计日志，X-Panel/Xray 字段保持只读。
...
- 节点摘要至少包含 `id`、`name`、`type`、`status`、`host`、`xrayVersion`、`lastSyncAt`、`monthlyCost`、`userCount`、`paidUserCount`、`freeUserCount`、`clientCount`；
