...
# Remove the Alien-Panel test deployment installed by this repository.
...
This permanently removes the Alien-Panel service, database, backups,
...
if command -v docker >/dev/null 2>&1; then
  compose_file="/opt/alien-panel/deploy/docker-compose.yml"
  if [[ -f "$compose_file" ]]; then
    echo "stop/remove Docker project: alien-panel"
    if (( CONFIRM == 1 )); then
      if docker compose version >/dev/null 2>&1; then
        docker compose -p alien-panel -f "$compose_file" down -v --remove-orphans || true
      elif command -v docker-compose >/dev/null 2>&1; then
        docker-compose -p alien-panel -f "$compose_file" down -v --remove-orphans || true
      fi
    fi
  fi
  if docker volume inspect alien-panel_central-data >/dev/null 2>&1; then
    echo "remove Docker volume: alien-panel_central-data"
    if (( CONFIRM == 1 )); then docker volume rm alien-panel_central-data || true; fi
  fi
  if docker image inspect alien-panel-central:local >/dev/null 2>&1; then
    echo "remove Docker image: alien-panel-central:local"
    if (( CONFIRM == 1 )); then docker image rm alien-panel-central:local || true; fi
  fi
fi
...
if (( CONFIRM == 1 )); then
  ...
  echo "Alien-Panel test deployment was removed. TLS certificates and node Agents were kept."
else
