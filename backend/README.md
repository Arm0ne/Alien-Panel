...
中央服务部署、节点接入、故障排查、备份恢复和灰度回滚请参阅包内的
...
- `GET /api/costs/summary?period=YYYY-MM` 返回本期实收、累计实收、服务期折算收入、节点/IP/其他真实成本、线路机承载和严格续费指标。已核实收费记录按服务区间分摊（年费按实际覆盖天数），历史导入但未核实的记录不计入现金或服务期收入；未有收费历史的旧用户继续回退到月费；成本按月份半开区间与记录有效区间求交集，避免下月生效成本提前计入。
...
- `GET /api/users/{id}/renewals` 查询 Agent 发现该 Inbound 的 Client 派生到期时间延长后生成的候选；`POST /api/users/{id}/renewals/{candidateId}/confirm` 确认收费并创建不可变收费记录；`POST .../reject` 标记为非收费变更。Agent 永远不会自动把到期延长直接计为收款。
- `POST /api/users/{id}/billing-records` 可手工新增首购、续费或逾期恢复订单；`POST /api/billing/import` 支持按用户 ID 或 `nodeId + remoteInboundId` 批量预览/导入历史订单。历史导入可标记为未核实，未核实记录保存为待处理状态，不计入现金和服务期收入，之后可通过 `POST .../billing-records/{recordId}/verify` 核验；错误或重复的手工/历史记录可通过 `POST .../cancel` 取消并保留审计历史。
...
已登记为 `relay` 的节点成功同步后，中央会将每个新发现的 Inbound 自动建立为一个业务用户，并关联其 Client/Email 设备。Inbound 仍保存自己的 X-Panel 到期字段，但中央业务用户的到期和状态只由已启用 Client 决定：相同时间直接使用，不同时间以最早有限时间为准并记录异常。同步不会覆盖中央维护的业务名称、月费、币种和备注。落地节点的 Inbound 不会自动创建业务用户。

Agent 存活与 X-Panel 数据采集分开判断。Agent 只要能够向中央发送心跳，节点状态即为在线；若本机 X-Panel 请求失败，节点显示 X-Panel 同步异常和错误详情，但不会被误判为离线。下一次完整同步成功会清除该异常。
