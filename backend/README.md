...
- `GET /api/costs/summary?period=YYYY-MM` 返回本期实收、累计实收、服务期折算收入、节点/IP/其他真实成本、线路机承载和严格续费指标。确认收费记录按服务区间分摊（年费按实际覆盖天数），未有收费历史的旧用户继续回退到月费；成本按月份半开区间与记录有效区间求交集，避免下月生效成本提前计入。
