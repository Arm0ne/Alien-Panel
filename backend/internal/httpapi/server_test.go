...
func TestAdministratorCanUpdateAccountAndSessionsAreRevoked(t *testing.T) {
	...
	status, invalid := doJSONWithStatus(t, ts.Client(), http.MethodPatch, ts.URL+"/api/auth/me", token, map[string]string{
		"currentPassword": "wrong-password",
		"userName":        "operator",
	})
	if status != http.StatusUnauthorized || invalid["code"] != unauthorizedCode {
		t.Fatalf("invalid current password response = %d %#v", status, invalid)
	}

	status, updated := doJSONWithStatus(t, ts.Client(), http.MethodPatch, ts.URL+"/api/auth/me", token, map[string]string{
		"currentPassword": "test-password",
		"userName":        "operator",
		"newPassword":     "new-password-123",
	})
	if status != http.StatusOK || updated["code"] != successCode {
		t.Fatalf("account update response = %d %#v", status, updated)
	}

	status, revoked := doJSONWithStatus(t, ts.Client(), http.MethodGet, ts.URL+"/api/auth/me", token, nil)
	if status != http.StatusUnauthorized || revoked["code"] != unauthorizedCode {
		t.Fatalf("old session response = %d %#v", status, revoked)
	}
	status, oldLogin := doJSONWithStatus(t, ts.Client(), http.MethodPost, ts.URL+"/api/auth/login", "", map[string]string{
		"userName": "admin",
		"password": "test-password",
	})
	if status != http.StatusUnauthorized || oldLogin["code"] != unauthorizedCode {
		t.Fatalf("old credentials response = %d %#v", status, oldLogin)
	}

	newLogin := doJSON(t, ts.Client(), http.MethodPost, ts.URL+"/api/auth/login", "", map[string]string{
		"userName": "operator",
		"password": "new-password-123",
	})
	newToken := newLogin["data"].(map[string]any)["token"].(string)
	userInfo := doJSON(t, ts.Client(), http.MethodGet, ts.URL+"/api/auth/me", newToken, nil)
	if userInfo["code"] != successCode || userInfo["data"].(map[string]any)["userName"] != "operator" {
		t.Fatalf("updated administrator info = %#v", userInfo)
	}
}
...
func TestAgentRegistrationRotatesPreviousToken(t *testing.T) {
	...
	var nodeID string
	if err := database.QueryRow(`SELECT id FROM nodes WHERE node_key = 'rotate-agent'`).Scan(&nodeID); err != nil {
		t.Fatalf("read registered node id: %v", err)
	}
	login := doJSON(t, ts.Client(), http.MethodPost, ts.URL+"/api/auth/login", "", map[string]string{"userName": "admin", "password": "test-password"})
	adminToken := login["data"].(map[string]any)["token"].(string)
	if status, response := doJSONWithStatus(t, ts.Client(), http.MethodPatch, ts.URL+"/api/nodes/"+nodeID, adminToken, map[string]any{"name": "中央自定义名称"}); status != http.StatusOK || response["code"] != successCode {
		t.Fatalf("rename node status=%d response=%#v", status, response)
	}
	...
	var nodeName string
	if err := database.QueryRow(`SELECT name FROM nodes WHERE node_key = 'rotate-agent'`).Scan(&nodeName); err != nil {
		t.Fatalf("read node name after re-registration: %v", err)
	}
	if nodeName != "中央自定义名称" {
		t.Fatalf("Agent re-registration overwrote central node name with %q", nodeName)
	}
...
func TestUserDetailAndBusinessFieldUpdate(t *testing.T) {
	...
	// A later relay snapshot may refresh Client-derived expiry/status, but must
	// never overwrite central-operated customer metadata.
	tx, err := database.Begin()
	if err != nil {
		t.Fatalf("begin relay sync transaction: %v", err)
	}
	if err := server.ensureRelayInboundUser(tx, "relay", "inbound-detail", "37", agentInboundPayload{
		Enable:  true,
		Remark:  "X-Panel changed this",
		Clients: []agentClientPayload{{RemoteID: "phone-uuid", Enable: true, ExpiryTime: newExpiryEpoch(expiryText)}},
...
func TestUserPathAssignmentLifecycle(t *testing.T) {
	...
	statements := []struct {
		...
		{`INSERT INTO exit_ips (id, source_type, owner_node_id, ip, family, enabled, created_at, updated_at) VALUES ('path-relay-ip-2', 'node', 'path-relay', '198.51.100.77', 4, 1, ?, ?)`, []any{nowText, nowText}},
	...
	if assetsData["relay"].(map[string]any)["id"] != "path-relay" || len(assetsData["relayExitIps"].([]any)) != 2 || len(assetsData["externalExitIps"].([]any)) != 1 {
	...
	multiRelay := doJSON(t, ts.Client(), http.MethodPut, ts.URL+"/api/users/path-user/path", token, map[string]any{"exitIpIds": []string{"path-relay-ip", "path-relay-ip-2"}, "notes": "多出口线路机直出"})
	if multiRelay["code"] != successCode {
		t.Fatalf("multi exit path response = %#v", multiRelay)
	}
	multiPath := multiRelay["data"].(map[string]any)["path"].(map[string]any)
	if multiPath["exitIpId"] != "path-relay-ip" || len(multiPath["exitIpIds"].([]any)) != 2 || len(multiPath["exitIps"].([]any)) != 2 {
		t.Fatalf("unexpected multi exit path = %#v", multiPath)
	}
	listWithMulti := doJSON(t, ts.Client(), http.MethodGet, ts.URL+"/api/users?page_size=20", token, nil)
	listItems := listWithMulti["data"].(map[string]any)["items"].([]any)
	if len(listItems) != 1 || listItems[0].(map[string]any)["exitIpAddress"] != "198.51.100.71, 198.51.100.77" {
		t.Fatalf("user list did not include all exit IPs: %#v", listWithMulti)
	}
	secondExitDeleteStatus, secondExitDelete := doJSONWithStatus(t, ts.Client(), http.MethodDelete, ts.URL+"/api/exit-ips/path-relay-ip-2", token, nil)
	if secondExitDeleteStatus != http.StatusConflict || secondExitDelete["code"] != validationCode {
		t.Fatalf("second exit IP assignment was not protected from deletion: status=%d response=%#v", secondExitDeleteStatus, secondExitDelete)
	}
	...
	if cleared["code"] != successCode || clearedData["path"] != nil || len(clearedData["pathHistory"].([]any)) != 4 {
	...
	payload := map[string]any{
		...
		"inbounds": []any{map[string]any{
			...
			"config_hash": "hash-1", "clients": []any{
				map[string]any{"remote_id": "client-a", "email": "phone", "enable": true, "expiry_time": 1792022400, "all_time": 300},
			},
		}},
	}
...
func TestDashboardUsesTrafficDeltas(t *testing.T) {
	...
	statements := []struct {
		...
		{`INSERT INTO users (id, display_name, status, created_at, updated_at) VALUES ('traffic-user', 'Traffic User', 'active', ?, ?)`, []any{created, created}},
		{`INSERT INTO inbounds (id, node_id, remote_inbound_id, user_id, kind, tag, first_seen_at, last_seen_at) VALUES ('traffic-inbound', 'traffic-node', '15', 'traffic-user', 'user', 'user-15', ?, ?)`, []any{first, second}},
		{`INSERT INTO user_inbounds (id, user_id, inbound_id, is_primary, active_from) VALUES ('traffic-mapping', 'traffic-user', 'traffic-inbound', 1, ?)`, []any{created}},
...
func TestNodeListIncludesCurrentRelayCapacity(t *testing.T) {
	server, database := testServer(t)
	now := time.Now().UTC()
	nowText := now.Format(time.RFC3339Nano)
	futureText := now.Add(24 * time.Hour).Format(time.RFC3339Nano)
	pastText := now.Add(-24 * time.Hour).Format(time.RFC3339Nano)
	statements := []struct {
		query string
		args  []any
	}{
		{`INSERT INTO nodes (id, node_key, name, type, health_status, created_at, updated_at) VALUES ('capacity-relay', 'capacity-relay', '承载线路机', 'relay', 'online', ?, ?)`, []any{nowText, nowText}},
		{`INSERT INTO nodes (id, node_key, name, type, health_status, created_at, updated_at) VALUES ('capacity-landing', 'capacity-landing', '承载落地机', 'landing', 'online', ?, ?)`, []any{nowText, nowText}},
		{`INSERT INTO users (id, display_name, status, billing_type, expiry_time, created_at, updated_at) VALUES ('capacity-paid', '收费用户', 'active', 'paid', ?, ?, ?)`, []any{futureText, nowText, nowText}},
		{`INSERT INTO users (id, display_name, status, billing_type, expiry_time, created_at, updated_at) VALUES ('capacity-free', '免费用户', 'active', 'free', ?, ?, ?)`, []any{futureText, nowText, nowText}},
		{`INSERT INTO users (id, display_name, status, billing_type, expiry_time, created_at, updated_at) VALUES ('capacity-expired', '过期用户', 'expired', 'paid', ?, ?, ?)`, []any{pastText, nowText, nowText}},
		{`INSERT INTO users (id, display_name, status, billing_type, created_at, updated_at) VALUES ('capacity-disabled', '停用用户', 'disabled', 'paid', ?, ?)`, []any{nowText, nowText}},
		{`INSERT INTO inbounds (id, node_id, remote_inbound_id, user_id, kind, tag, client_count, first_seen_at) VALUES ('capacity-inbound-paid', 'capacity-relay', 'paid', 'capacity-paid', 'user', 'paid', 1, ?)`, []any{nowText}},
		{`INSERT INTO inbounds (id, node_id, remote_inbound_id, user_id, kind, tag, client_count, first_seen_at) VALUES ('capacity-inbound-free', 'capacity-relay', 'free', 'capacity-free', 'user', 'free', 2, ?)`, []any{nowText}},
		{`INSERT INTO inbounds (id, node_id, remote_inbound_id, user_id, kind, tag, client_count, first_seen_at) VALUES ('capacity-inbound-expired', 'capacity-relay', 'expired', 'capacity-expired', 'user', 'expired', 1, ?)`, []any{nowText}},
		{`INSERT INTO inbounds (id, node_id, remote_inbound_id, user_id, kind, tag, client_count, first_seen_at) VALUES ('capacity-inbound-disabled', 'capacity-relay', 'disabled', 'capacity-disabled', 'user', 'disabled', 1, ?)`, []any{nowText}},
		{`INSERT INTO user_inbounds (id, user_id, inbound_id, is_primary, active_from) VALUES ('capacity-map-paid', 'capacity-paid', 'capacity-inbound-paid', 1, ?)`, []any{nowText}},
		{`INSERT INTO user_inbounds (id, user_id, inbound_id, is_primary, active_from) VALUES ('capacity-map-free', 'capacity-free', 'capacity-inbound-free', 1, ?)`, []any{nowText}},
		{`INSERT INTO user_inbounds (id, user_id, inbound_id, is_primary, active_from) VALUES ('capacity-map-expired', 'capacity-expired', 'capacity-inbound-expired', 1, ?)`, []any{nowText}},
		{`INSERT INTO user_inbounds (id, user_id, inbound_id, is_primary, active_from) VALUES ('capacity-map-disabled', 'capacity-disabled', 'capacity-inbound-disabled', 1, ?)`, []any{nowText}},
		{`INSERT INTO clients (id, node_id, inbound_id, remote_client_id, email, enable) VALUES ('capacity-client-paid', 'capacity-relay', 'capacity-inbound-paid', 'phone', 'paid@example.com', 1)`, nil},
		{`INSERT INTO clients (id, node_id, inbound_id, remote_client_id, email, enable) VALUES ('capacity-client-free', 'capacity-relay', 'capacity-inbound-free', 'laptop', 'free@example.com', 1)`, nil},
		{`INSERT INTO clients (id, node_id, inbound_id, remote_client_id, email, enable) VALUES ('capacity-client-disabled', 'capacity-relay', 'capacity-inbound-disabled', 'old', 'disabled@example.com', 0)`, nil},
	}
	for _, statement := range statements {
		if _, err := database.Exec(statement.query, statement.args...); err != nil {
			t.Fatalf("seed capacity data: %v", err)
		}
	}
	...
	login := doJSON(t, ts.Client(), http.MethodPost, ts.URL+"/api/auth/login", "", map[string]string{"userName": "admin", "password": "test-password"})
	token := login["data"].(map[string]any)["token"].(string)
	result := doJSON(t, ts.Client(), http.MethodGet, ts.URL+"/api/nodes?page_size=20", token, nil)
	if result["code"] != successCode {
	...
	relay := byID["capacity-relay"]
	if relay["userCount"] != float64(2) || relay["paidUserCount"] != float64(1) || relay["freeUserCount"] != float64(1) || relay["clientCount"] != float64(2) {
		t.Fatalf("relay capacity = %#v, want users=2 paid=1 free=1 clients=2", relay)
	}
	if _, present := relay["xpanelVersion"]; present {
		t.Fatalf("node list still exposes removed xpanelVersion field: %#v", relay)
	}
	if _, present := relay["lastSeenAt"]; present {
		t.Fatalf("node list still exposes removed node lastSeenAt field: %#v", relay)
	}
	landing := byID["capacity-landing"]
	if landing["userCount"] != float64(0) || landing["clientCount"] != float64(0) {
		t.Fatalf("landing capacity = %#v, want zero", landing)
	}
}
...
func TestNodeDetailAndManualSyncRequest(t *testing.T) {
	...
	statements := []struct {
		...
		{`INSERT INTO inbounds (id, node_id, remote_inbound_id, kind, tag, protocol, port, enable, client_count, up, down, all_time, first_seen_at, last_seen_at, missing_since, missing_sync_count) VALUES ('node-missing-inbound', 'node-detail', '8', 'infrastructure', 'ss-missing', 'shadowsocks', 8444, 1, 2, 1, 2, 3, ?, ?, ?, 1)`, []any{nowText, nowText, nowText}},
		{`INSERT INTO inbounds (id, node_id, remote_inbound_id, kind, tag, protocol, port, enable, client_count, up, down, all_time, first_seen_at, last_seen_at, deleted_at) VALUES ('node-archived-inbound', 'node-detail', '9', 'infrastructure', 'ss-deleted', 'shadowsocks', 8444, 1, 2, 1, 2, 3, ?, ?, ?)`, []any{nowText, nowText, nowText}},
	...
	if _, present := detailData["xpanelVersion"]; present {
		t.Fatalf("node detail still exposes removed xpanelVersion field: %#v", detailData)
	}
	if _, present := detailData["lastSeenAt"]; present {
		t.Fatalf("node detail still exposes removed node lastSeenAt field: %#v", detailData)
	}
...
func TestExitIPCRUDAndBindingProtection(t *testing.T) {
	...
	seed := []struct {
		...
		{`INSERT INTO nodes (id, node_key, name, type, region, health_status, created_at, updated_at) VALUES ('exit-landing', 'exit-landing', '落地机 B', 'landing', '东京', 'online', ?, ?)`, []any{nowText, nowText}},
		{`INSERT INTO nodes (id, node_key, name, type, region, health_status, created_at, updated_at) VALUES ('exit-relay', 'exit-relay', '线路机 B', 'relay', '东京', 'online', ?, ?)`, []any{nowText, nowText}},
	...
	if createdData["region"] != "东京" {
		t.Fatalf("node exit IP region = %#v, want 东京", createdData["region"])
	}
	...
	if listedExitIP["region"] != "东京" {
		t.Fatalf("exit IP list region = %#v, want 东京", listedExitIP["region"])
	}
	...
	if detail["data"].(map[string]any)["region"] != "东京" {
		t.Fatalf("exit IP detail region = %#v, want 东京", detail["data"].(map[string]any)["region"])
	}
	if status, response := doJSONWithStatus(t, ts.Client(), http.MethodPatch, ts.URL+"/api/nodes/exit-landing", token, map[string]any{"region": "大阪"}); status != http.StatusOK || response["code"] != successCode {
		t.Fatalf("update owner node region status=%d response=%#v", status, response)
	}
	updatedRegion := doJSON(t, ts.Client(), http.MethodGet, ts.URL+"/api/exit-ips/"+exitIPID, token, nil)
	if updatedRegion["data"].(map[string]any)["region"] != "大阪" {
		t.Fatalf("exit IP region after node update = %#v, want 大阪", updatedRegion["data"].(map[string]any)["region"])
	}
	updatedList := doJSON(t, ts.Client(), http.MethodGet, ts.URL+"/api/exit-ips?page=1&page_size=20&keyword=%E5%A4%A7%E9%98%AA", token, nil)
	updatedListItems := updatedList["data"].(map[string]any)["items"].([]any)
	if updatedList["code"] != successCode || len(updatedListItems) != 1 || updatedListItems[0].(map[string]any)["region"] != "大阪" {
		t.Fatalf("exit IP list should search by derived region: %#v", updatedList)
	}
	...
	missingS5Region := doJSON(t, ts.Client(), http.MethodPost, ts.URL+"/api/exit-ips", token, map[string]any{
		"address": "203.0.113.44", "sourceType": "s5", "family": 4,
	})
	if missingS5Region["code"] != validationCode {
		t.Fatalf("missing S5 region response = %#v", missingS5Region)
	}
...
func TestRouteExitIPBindingCRUD(t *testing.T) {
	...
	seed := []struct {
		...
		{`INSERT INTO nodes (id, node_key, name, type, region, health_status, created_at, updated_at) VALUES ('binding-relay', 'binding-relay', '绑定线路机', 'relay', '东京', 'online', ?, ?)`, []any{nowText, nowText}},
		{`INSERT INTO nodes (id, node_key, name, type, region, health_status, created_at, updated_at) VALUES ('binding-landing-1', 'binding-landing-1', '绑定落地机 1', 'landing', '东京', 'online', ?, ?)`, []any{nowText, nowText}},
		{`INSERT INTO nodes (id, node_key, name, type, region, health_status, created_at, updated_at) VALUES ('binding-landing-2', 'binding-landing-2', '绑定落地机 2', 'landing', '大阪', 'online', ?, ?)`, []any{nowText, nowText}},
	...
	if boundData["region"] != "东京" {
		t.Fatalf("bound route exit IP region = %#v, want 东京", boundData["region"])
	}
