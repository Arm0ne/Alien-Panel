...
FROM nodes WHERE id = ? AND deleted_at IS NULL`, id).Scan(&existing.NodeKey, &existing.Name, &existing.Type, &existing.Hostname, &existing.ManagementURL, &existing.PublicIP, &existing.Region, &existing.Provider, &existing.PanelBasePath, &enabled, &existing.HealthStatus)
	...
	// Older Agent registrations could leave panel_base_path empty. Treat that
	// legacy value as the root path so an otherwise unrelated metadata update
	// (such as renaming a node) remains valid and repairs the record safely.
	if existing.PanelBasePath == "" {
		existing.PanelBasePath = "/"
	}
...
func (s *Server) deleteNode(w http.ResponseWriter, r *http.Request) {
	...
	// Capture users before deleting node-owned associations. After cleanup we
	// re-check each user against every remaining relay/landing association,
	// then logically delete only those that have become orphaned.
	affectedUserIDs, err := nodeAffectedUserIDsTx(tx, id)
	if err != nil {
		s.logger.Error("find users affected by node deletion", "node_id", id, "error", err)
		writeFailure(w, http.StatusInternalServerError, internalErrorCode, "could not read node users")
		return
	}
	...
	if err := execDelete("user paths", `DELETE FROM user_paths WHERE relay_node_id = ? OR landing_node_id = ? OR landing_inbound_id IN (SELECT id FROM inbounds WHERE node_id = ?) OR exit_ip_id IN (SELECT id FROM exit_ips WHERE owner_node_id = ? OR landing_node_id = ?) OR EXISTS (SELECT 1 FROM user_path_exit_ips upi JOIN exit_ips e ON e.id = upi.exit_ip_id WHERE upi.user_path_id = user_paths.id AND (e.owner_node_id = ? OR e.landing_node_id = ?))`, id, id, id, id, id, id, id); err != nil {
	...
	if err := s.autoDeleteOrphanedUsersTx(tx, r, affectedUserIDs, id, name, time.Now().UTC()); err != nil {
		s.logger.Error("auto-delete orphaned users after node deletion", "node_id", id, "error", err)
		writeFailure(w, http.StatusInternalServerError, internalErrorCode, "could not clean up orphaned users")
		return
	}
	...
	// Renewal events are business history. Detach them from the node being
	// removed, so the generic operational-event cleanup below cannot erase a
	// pending or resolved billing decision.
	if err := execDelete("renewal events", `UPDATE node_events SET node_id = NULL WHERE node_id = ? AND resource_type = 'renewal'`, id); err != nil {
		writeFailure(w, http.StatusInternalServerError, internalErrorCode, "could not preserve renewal events")
		return
	}
