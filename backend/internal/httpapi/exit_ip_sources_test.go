...
func TestExitIPSourcesAndRouteScopes(t *testing.T) {
	...
	s5IP := create(map[string]any{"address": "198.51.100.72", "sourceType": "s5", "region": "新加坡", "provider": "S5 Supplier"})
	...
	if s5Detail["data"].(map[string]any)["region"] != "新加坡" {
		t.Fatalf("S5 exit region = %#v, want 新加坡", s5Detail["data"].(map[string]any)["region"])
	}
}
