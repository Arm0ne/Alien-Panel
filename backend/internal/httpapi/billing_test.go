...
func TestAgentExpiryExtensionCreatesConfirmableRenewal(t *testing.T) {
	...
	if err := server.ensureRelayInboundUser(tx, "relay", "billing-inbound", "1", agentInboundPayload{
		Enable:  true,
		Clients: []agentClientPayload{{RemoteID: "billing-client", Enable: true, ExpiryTime: newExpiryEpoch(newExpiry)}},
