...
- 用户详情可维护月付/年付及周期价格。Agent 发现线路机 Inbound 到期时间延长时只生成“待确认续费”，不会自动算作收款；管理员确认后才写入收费历史并进入财务统计，选择“非收费变更”则只关闭候选。
- 年费按收费记录的实际服务区间分摊到各统计月份；未产生收费记录的旧用户继续使用 `monthly_fee` 兼容口径。删除或停用节点不会删除用户收费历史。
- “事件中心”只展示业务事件和有效告警。立即同步请求保留在内部 `sync_runs`/内部事件中，不作为管理员事件列表；同步失败、节点离线/恢复、流量回退和 Inbound 异常会形成可检索事件。
